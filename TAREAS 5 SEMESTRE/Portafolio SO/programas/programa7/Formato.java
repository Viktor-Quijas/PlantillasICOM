...
public class Formato extends JDialog{
	...
	public Formato(Frame padre, boolean modal) {	//	Parametros necesarios para superponer la ventana
		...
		panelEntrada = new JPanel(new GridLayout(2, 2, 10, 10));					//	<-- Se crea el panel para los campos con un márgen determinado				
		...
		JLabel lblQuantum = new JLabel("Ingresa el quantum porfi:");	//	<-- Label para el nombre del campo
		JTextField txtQuantum = new JTextField();							// 	<-- Cuadro de texto
		
		panelEntrada.add(lblProcesos);	panelEntrada.add(txtCantProcesos);		//	<-- Se añaden al panel de entrada de datos
		panelEntrada.add(lblQuantum);	panelEntrada.add(txtQuantum);		//	<-- Se añaden al panel de entrada de datos
		...
		btnContinuar.addActionListener(e -> {			//Función lambda
			...
			String strQuantum = txtQuantum.getText();

			//	-- Verificaciones y paso de string a integers --
			if (!strCantProcesos.isEmpty() && !strQuantum.isEmpty()) {
				try {
					cantidadTotalProcesos = Integer.parseInt(strCantProcesos);
					quantum = Integer.parseInt(strQuantum);
					// 	-- Verificaciones de entradas --
					if (cantidadTotalProcesos > 0 && quantum > 0) {
