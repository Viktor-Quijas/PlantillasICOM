		...
		for (int i = 0; i < 8; i++) {
			for (int j = 0; j < 6; j++) {
				if (memoriaFisica[contMemoria] != null) {
					lblMemoriaFisica[i][j].setText(memoriaFisica[contMemoria].id + " [ " + memoriaFisica[contMemoria].getPaginaInfo()[Proceso.LONGITUD_PAG] + "/5 ]");
					...
					if (buscarProcesoEnMemoriaLogica(MEMORIA, memoriaFisica[contMemoria])) {
						...
						lblMemoriaFisica[i][j].setText("SO");
						lblMemoriaFisica[i][j].setForeground(Color.LIGHT_GRAY);
					}
				} else {
					lblMemoriaFisica[i][j].setText("[ 0/5 ]");
