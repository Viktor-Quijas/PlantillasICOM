...
public class Tabla extends JFrame{
	...
	private static final int LIMPIAR_VENTANA = 10;
	...
	private void InicializarVariables() {
		procesoId = procesos.size();
		reloj = new Timer(500, e -> {
	...
	private void IngresarAEjecucion() {
		...
		if (!procesador.get(0).seEjecuto) {
			...
			procesador.get(0).tiempoRespuesta = contadorGlobal - procesador.get(0).tiempoRespuesta;
		}
	}
	...
	private void Ejecucion() {
		if (!colaEventos.contains(INTERCAMBIO_DE_PROCESOS)) {
	...
	public void Continuar() { reloj.start(); }
	
	public void Pausa() { reloj.stop(); }
	...
	private void ContinuarEjecucion() {
		...
		if (procesador.get(0).tiempoTranscurrido == procesador.get(0).tiempoMax) {
			...
			if (procesos.isEmpty() && memoria.isEmpty() && bloqueados.isEmpty())
	...
	private void AgregarProcesoNuevo() { procesos.add(CrearProceso()); }
