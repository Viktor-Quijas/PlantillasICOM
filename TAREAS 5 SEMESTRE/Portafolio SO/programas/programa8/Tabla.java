...
public class Tabla extends JFrame{
	...
	public static final int MARGEN_X = 150;
	public static final int MARGEN_Y = 60;
	...
	private static final int SUSPENDER_PROCESO = 12;
	private static final int REGRESAR_PROCESO = 13;
	...
	private List<Proceso> suspendidos = new ArrayList<>();
	private List<Proceso> aRegresar = new ArrayList<>();
	...
	private TablaSuspendidos tablaSuspendidos;
	...
	//	-- Encargado de escuchar si una tecla fue presionada --
			this.addKeyListener(new KeyAdapter() {
				...
				public void keyPressed(KeyEvent e) {
					...
					case KeyEvent.VK_S:
						if (relojLogico) {
							colaEventos.add(SUSPENDER_PROCESO);
						}
						break;
					case KeyEvent.VK_R:
						if (relojLogico) {
							colaEventos.add(REGRESAR_PROCESO);
						}
						break;
					}
				}
	...
	private void InicializarTablasAdicionales() {
		...
		tablaSuspendidos = new TablaSuspendidos(this);
		tablaSuspendidos.setVisible(true);
	}
	...
	private void IntegrarListas() {
		...
		registroListas.add(suspendidos); //Cambiar la posicion
		registroListas.add(aRegresar); //Cambiar la posicion
	...
	private void IngresarAMemoria() {
		if (procesos.isEmpty() && aRegresar.isEmpty())
			return;
		
		List<Proceso> prioridad = new ArrayList<>();
		prioridad.addAll(aRegresar);
		prioridad.addAll(procesos);
		
		Proceso siguienteEnMemoria;
		
		while (!prioridad.isEmpty() && hayMarcosDisponibles(prioridad.get(0).getCantidadPaginas())) {
			siguienteEnMemoria = prioridad.get(0);
			
			if (ingresarMemoriaFisica(siguienteEnMemoria)) {
				memoriaVirtual.add(siguienteEnMemoria);
				
				if (!memoriaVirtual.get(memoriaVirtual.size() - 1).seIngresoAMemoria) {
					memoriaVirtual.get(memoriaVirtual.size() - 1).tiempoLlegada = contadorGlobal;
					memoriaVirtual.get(memoriaVirtual.size() - 1).seIngresoAMemoria = true;
				}
				
				agregarFilaParaMemoria();
				
				if (!aRegresar.isEmpty() && aRegresar.get(0) == prioridad.get(0))
					aRegresar.remove(0);
				
				else if (!procesos.isEmpty() && procesos.get(0) == prioridad.get(0))
					procesos.remove(0);
	...
	private void suspenderProceso() {
		...
		Proceso suspendido = bloqueados.get(0);
		bloqueados.remove(0);
		suspendido.bloqueado = false;
		tablaBloqueados.ActualizarTabla();
		eliminarDeMemoriaFisica(suspendido);
		suspendidos.add(suspendido);
		imprimirDoc();
		IngresarAMemoria();
		RefrescarTabla();
	}
	
	private void regresarProceso() {
		if (suspendidos.isEmpty()) 
			return;
		
		Proceso porRegresar = suspendidos.get(0);
		aRegresar.add(porRegresar);
		suspendidos.remove(porRegresar);
		IngresarAMemoria();
		RefrescarTabla();
	}
	...
	private void imprimirDoc() {
		...
		List<String[]> informacionSuspendidos = new ArrayList<>();
		List<Proceso> procesosSuspendidos = new ArrayList<>(aRegresar);
		procesosSuspendidos.addAll(suspendidos);
		
		for (int i = 0; i < procesosSuspendidos.size(); i++) {
			String[] informacion = new String[3];
			informacion[ID] = String.valueOf(procesosSuspendidos.get(i).id);
			informacion[LONG] = String.valueOf(procesosSuspendidos.get(i).getLongitud());
			informacion[TRNCPU] = String.valueOf(procesosSuspendidos.get(i).tiempoMax - procesosSuspendidos.get(i).tiempoTranscurrido);
			
			informacionSuspendidos.add(informacion);
		}
		
		try (PrintWriter escritor = new PrintWriter(new FileWriter("procesosSuspendidos.txt"))) {
            escritor.println("Procesos Suspendidos:");
            
            for (int i = 0; i < informacionSuspendidos.size(); i++)	
            	escritor.println("ID: " + informacionSuspendidos.get(i)[ID] + 
            			"\nTamaño: " + informacionSuspendidos.get(i)[LONG] + 
            			"\nTiempo restante en CPU: " + informacionSuspendidos.get(i)[TRNCPU] + 
            			"\n");
	...
	private void LlamadaATerminar() {
		if (procesador.isEmpty() && memoriaVirtual.isEmpty() && bloqueados.isEmpty() && procesos.isEmpty() && suspendidos.isEmpty() && aRegresar.isEmpty())
