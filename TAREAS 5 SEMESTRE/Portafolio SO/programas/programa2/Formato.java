/*
 * 			CAMBIOS VERSION 2.0
 * - PROPIEDADES DORMIDAS: Todas aquellas propiedades que no se necesitan de la versión anterior estan comentadas, ejem. Pedir datos al usuario.
 * - MÉTODOS DORMIDOS: El antiguo constructor fue cambiado por uno más sencillo, se duerme la función encargad de 
 * guardar los procesos en pilas pues la función automática se encarga de ello.
 */
...
public class Formato extends JDialog{
	...
	private int cantProcesos;			// Cantidad de procesos que se van a generar.
	
	//Listas
	/*private ArrayList<Integer> ids = new ArrayList<>();
	private List<Proceso> lote = new ArrayList<>();
	...
	public Formato(Frame padre, boolean modal) {	//	Parametros necesarios para superponer la ventana
		...
		panelEntrada = new JPanel(new GridLayout(1, 2, 1, 1));					//	<-- Se crea el panel para los botones con un márgen determinado				
		...
		btnContinuar.addActionListener(e -> {			//Función lambda
			...
			if (!strCantProcesos.isEmpty()) {
				try {
					...
					cantProcesos = Integer.parseInt(strCantProcesos);
	...
	*/
	...
	public List<Lote> GetLista()		{ return generarListaDeLotes(cantProcesos);	}		//	<-- La versión 1.0 retorna listaLotes.
	public boolean GetFinalizado()		{ return finalizado;			}
	
	
	//	-- Método para generar procesos de manera aut. de la Ver 2.0 --
	public List<Lote> generarListaDeLotes(int cantidadTotalProcesos) {
        List<Lote> listaLotes = new ArrayList<>();
        List<Proceso> procesosTemporales = new ArrayList<>();
        ...
        for (int i = 1; i <= cantidadTotalProcesos; i++) {
            ...
            procesosTemporales.add(p);

            // Si ya tenemos 5 procesos o es el último proceso de la lista total
            if (procesosTemporales.size() == 5 || i == cantidadTotalProcesos) {
                // Creamos el lote y lo añadimos a la lista final
                listaLotes.add(new Lote(new ArrayList<>(procesosTemporales)));
                
                // Limpiamos la lista temporal para el siguiente lote
                procesosTemporales.clear();
            }
